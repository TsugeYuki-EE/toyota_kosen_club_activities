...
export default async function TableTennisNotesPage({ searchParams }: PageProps) {
  ...
  const notes = await prisma.tableTennisNote.findMany({
    ...
    orderBy: [{ createdAt: "desc" }, { noteDateKey: "desc" }],
  });
      ...
      <main className={styles.main}>
        <section className={styles.card}>
          ...
          <div style={{ marginTop: 12 }}>
            <Link href="/table-tennis-notes/new" className={styles.secondary} style={{ marginLeft: 12 }}>新しいノートを書く</Link>
          </div>
        </section>
        ...
        <section className={styles.card}>
          <h2>ノート一覧</h2>
          {notes.length === 0 ? (
            <p className={styles.muted}>まだノートはありません。新しいノートを書いてみましょう。</p>
          ) : (
            <ul className={styles.list}>
              {notes.map((note: { id: string; noteDateKey: string; content: string; createdAt: Date }) => (
                <li key={note.id} className={styles.listItem}>
                  ...
                  <p className={styles.muted} style={{ marginTop: 0 }}>
                    作成日時: <LocalDateTime value={note.createdAt} />
                  </p>
                  <div>
